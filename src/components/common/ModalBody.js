      ...
      if (Array.isArray(arr) || Array.isArray(arr.rowData)) {
        ...
        return (
          <div className = "modal-wrapper" style={{width:`${width}`}}>
            {newArr.map((ele) => {
              return mapping(ele, newHeight, align, newHeaderHeight)
            })}
          </div>
        )
      } else {
        ...
        switch (arr.type) {
          case 'IMAGE':
            modalCmp = (
            <div className="row-content row-image" style={{width:`${arr.width}`}}>
              <img style={{width:`100%`, height:`206px`}} src={cardData === null ? null :`${cardData[arr.key]}`}/>
            </div>
            )
            break;
          ...
          default:
            modalCmp = (
              <div className="row-content " style={{width:`${arr.width}`, height:`${height}`}}>
                <div className="top-row-content" style={{backgroundColor:`${arr.color}`, height: `${headerHeight}`}}>
                  {arr.title}
                </div>
                <div className="bottom-row-content" style={{textAlign: `${align}`}}>
                  {cardData === null ? 
